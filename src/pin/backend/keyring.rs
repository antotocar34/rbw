...
impl PinBackend for KeyringPinBackend {
    ...
    fn retrieve_local_secret(&self, _: &KeyringConfig) -> anyhow::Result<crate::locked::Vec> {
    ...
    fn store_local_secret(&self, kek: &crate::locked::Vec, _: &KeyringConfig) -> anyhow::Result<()> {
        let entry = Entry::new("rbw", crate::dirs::profile().as_str())?;
    ...
    fn clear_local_secret(&self) -> anyhow::Result<()> {
        let entry = Entry::new("rbw", crate::dirs::profile().as_str())?;
        entry.delete_credential().map_err(|_| anyhow::anyhow!("Could not delete credential"))
    }
}